function data = sampling_lpvextrema(lpv, dep, domain, gridsize,chebfextrema)
...
if Sy == 1 || Sx == 1
	for i = 1:length(lpv)
		...
		% sampling
		data{i} = sampling_depextrema(lpv{i}, depidx, domain(depidx,:), gridsize(depidx),chebfextrema);
	end
else
	for i = 1:Sy
		for j = 1:Sx
			...
			% sampling
			data{i,j} = sampling_depextrema(lpv{i,j}, depidx, domain(depidx,:), gridsize(depidx),chebfextrema);
		end
	end
end
